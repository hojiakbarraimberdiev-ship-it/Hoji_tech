...
const LISTINGS_KEY = 'hoji_tech_listings';
...
export function addListing(listing) {
  ...
  const newListing = { id: nextId, createdAt: new Date().toISOString(), ...listing };
  listings.unshift(newListing);
  saveListings(listings);
  return newListing;
}
export function getListingById(id) {
  return getListings().find(item => Number(item.id) === Number(id));
}
