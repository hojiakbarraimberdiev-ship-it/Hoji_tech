...
import { debounce, getUrlParams, formatDate, validateEmail,
  ...
  parseSpecs, parseImageUrls, readFileAsDataURL, confirmAction,
  getListings, addListing, getListingById } from './utils.js';
...
function initCatalogPage() {
  ...
  function renderProducts(page = 1) {
    ...
    renderCatalogListings();
  }

  function renderCatalogListings() {
    const grid = document.getElementById('catalogListingsGrid');
    const emptyEl = document.getElementById('catalogListingsEmpty');
    if (!grid) return;
    const listings = getListings().slice(0, 6);
    ...
    grid.innerHTML = listings.map(listing => `
      <article class="listing-card">
        <div class="listing-card__image" style="background-image:url('${listing.images?.[0] || 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=900&q=80'}')"></div>
        <div class="listing-card__body">
          <div class="listing-card__top">
            <span class="badge ${listing.condition === 'new' ? 'badge--new' : 'badge--used'}">${listing.condition === 'new' ? 'Новый' : 'Б/У'}</span>
            <span>${listing.type === 'pc' ? 'ПК' : 'Ноутбук'}</span>
          </div>
          ...
          <div class="listing-card__info">
            ...
            <span class="listing-card__date">${formatDate(listing.createdAt)}</span>
          </div>
          <div class="listing-card__meta">
            <span>Телефон: +996 ${listing.phone}</span>
            ${listing.location ? `<span>Город: ${listing.location}</span>` : ''}
          </div>
        </div>
      </article>
    `).join('');
...
function initListingsPage() {
  ...
  const searchClear = document.getElementById('listingSearchClear');
  ...
  function renderListings() {
    ...
    if (filters.search) {
      listings = listings.filter(listing =>
        ...
        listing.description.toLowerCase().includes(filters.search) ||
    ...
    grid.innerHTML = listings.map(listing => `
      <article class="product-card">
        <div class="product-card__img-wrap">
          <img src="${listing.images?.[0] || 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=900&q=80'}" alt="${listing.title}" class="product-card__img">
          <div class="product-card__badges">
            <span class="badge ${listing.condition === 'new' ? 'badge--new' : 'badge--used'}">${listing.condition === 'new' ? 'Новый' : 'Б/У'}</span>
            <span class="badge badge--sale">${listing.type === 'pc' ? 'ПК' : 'Ноутбук'}</span>
          </div>
        </div>
        <div class="product-card__body">
          <span class="product-card__brand">${listing.location || 'Частное'}</span>
          <h2 class="product-card__title">${listing.title}</h2>
          <p class="product-card__desc">${listing.description}</p>
          <div class="product-card__price-row">
            <span class="product-card__price">${formatPrice(listing.price)}</span>
            ${listing.phone ? `<span class="product-card__price-old">+996 ${listing.phone}</span>` : ''}
          </div>
  ...
  if (searchInput) {
    const debounced = debounce(() => renderListings(), 250);
    searchInput.addEventListener('input', () => {
      if (searchClear) searchClear.style.display = searchInput.value ? 'block' : 'none';
      debounced();
    });
...
function initBrandsPage() {
  ...
  const cards = Array.from(grid.querySelectorAll('.product-card')).map((card, index) => ({
    card,
  ...
  function renderBrands() {
    ...
    let visible = cards.filter(item => {
      ...
      return [item.title, item.desc, item.brand].some(text => text.toLowerCase().includes(query));
    });

    if (sort === 'name_asc') {
      visible.sort((a, b) => a.title.localeCompare(b.title, 'ru'));
    } else if (sort === 'name_desc') {
      visible.sort((a, b) => b.title.localeCompare(a.title, 'ru'));
    } else {
      visible.sort((a, b) => a.index - b.index);
    }

    cards.forEach(item => item.card.style.display = 'none');
  ...
  sortSelect?.addEventListener('change', renderBrands);
...
function initSellPage() {
  ...
  const emailInput = document.getElementById('listingEmail');
  const currentUser = getCurrentUser();
  if (currentUser) {
    if (phoneInput && currentUser.phone) phoneInput.value = currentUser.phone.replace(/^\+?996/, '');
  ...
  form.addEventListener('submit', async e => {
    ...
    const type = document.getElementById('listingType')?.value || 'laptop';
    const condition = document.getElementById('listingCondition')?.value || 'used';
    ...
    const imageText = document.getElementById('listingImages')?.value || '';
    const fileInput = document.getElementById('listingImageFiles');

    let valid = true;
    if (!title) { showFieldError('listingTitleError', 'Введите заголовок объявления'); valid = false; } else clearFieldError('listingTitleError');
    if (price <= 0) { showFieldError('listingPriceError', 'Введите корректную цену'); valid = false; } else clearFieldError('listingPriceError');
    if (!description) { showFieldError('listingDescriptionError', 'Опишите товар'); valid = false; } else clearFieldError('listingDescriptionError');
    if (!phone || !utilValidatePhone(phone)) { showFieldError('listingPhoneError', 'Введите телефон из 9 цифр'); valid = false; } else clearFieldError('listingPhoneError');
    if (email && !validateEmail(email)) { showFieldError('listingEmailError', 'Введите корректный email'); valid = false; } else clearFieldError('listingEmailError');
    if (!valid) return;

    let images = parseImageUrls(imageText);
    ...
    addListing({
      ...
      sellerName: currentUser ? `${currentUser.firstName} ${currentUser.lastName}` : 'Гость'
    });
...
function initAdminPage() {
  ...
  document.querySelectorAll('.admin-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tab.dataset.tab === 'listings') renderListingsTable();
    });
  });
  ...
  form?.addEventListener('submit', async e => {
    ...
    // Handle uploaded files (new feature)
    let uploadedImages = [];
    ...
    if (fileInput?.files?.length > 0) {
      ...
      for (const file of fileInput.files) {
        if (file.size > 2 * 1024 * 1024) { // 2MB limit per file
          showToast(`Файл ${file.name} слишком большой (макс 2MB)`, 'error');
          return;
        }
        totalSize += file.size;
        if (totalSize > 5 * 1024 * 1024) { // 5MB total limit
          showToast('Общий размер файлов слишком большой (макс 5MB)', 'error');
          return;
        }
        const dataUrl = await readFileAsDataURL(file);
    ...
    const data = {
      ...
      images: [...parseImageUrls(document.getElementById('pImages')?.value || ''), ...uploadedImages],
      inStock: document.getElementById('pInStock')?.checked
    };

    // Check total localStorage size
    const existingProducts = getAllProducts();
    const tempProducts = id ? existingProducts.map(p => p.id == id ? data : p) : [...existingProducts, { ...data, id: Date.now() }];
    const tempStorage = JSON.stringify(tempProducts);
    if (tempStorage.length > 4 * 1024 * 1024) { // 4MB limit for products
      showToast('Слишком много данных. Удалите старые товары или используйте внешние URLs.', 'error');
      return;
    }
  ...
  function renderProductsTable() {
    ...
    tbody.querySelectorAll('[data-edit]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // Clear file input for edit (files not supported in edit)
        document.getElementById('pImageFiles').value = '';
        openModal();
      });
    });
  ...
  function renderListingsTable() {
    ...
    if (!tbody) return;
    const listings = [...getListings()].reverse();
    if (listings.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--text-3)">Объявлений нет</td></tr>';
      return;
    }
    tbody.innerHTML = listings.map(l => `
      <tr>
        ...
        <td>${l.phone || '—'}</td>
        <td>${l.type === 'pc' ? 'ПК' : 'Ноутбук'}</td>
        <td><span style="font-family:var(--font-mono)">${formatPrice(l.price)}</span></td>
        <td><span class="badge ${l.condition === 'new' ? 'badge--new' : 'badge--used'}">${l.condition === 'new' ? 'Новое' : 'Б/У'}</span></td>
        <td>${formatDate(l.createdAt)}</td>
        <td>
          <button class="admin-btn admin-btn--edit" data-listing-id="${l.id}">Цена</button>
          <button class="admin-btn admin-btn--status" data-listing-id="${l.id}" data-action="add">+ Каталог</button>
          <button class="admin-btn admin-btn--delete" data-listing-id="${l.id}" data-action="delete">Удалить</button>
        </td>
      </tr>
    `).join('');
      ...
      if (btn.dataset.action === 'delete') {
        btn.addEventListener('click', () => {
          ...
          if (confirm('Удалить объявление?')) {
            let listings = getListings().filter(l => String(l.id) !== String(listingId));
            localStorage.setItem('itmarket_listings', JSON.stringify(listings));
            renderListingsTable();
        ...
        btn.addEventListener('click', () => {
          const listingId = btn.dataset.listingId;
          const listing = getListings().find(l => String(l.id) === String(listingId));
          if (!listing) {
            showToast('Объявление не найдено', 'error');
            return;
          }
          const newProduct = {
            id: Math.max(0, ...getAllProducts().map(p => p.id)) + 1,
            name: listing.title,
            brand: 'Частное',
            type: listing.type,
          ...
          showToast('Добавлено в каталог', 'success');
          renderListingsTable();
        });
      } else {
        btn.addEventListener('click', () => {
          ...
          const newPrice = prompt(`Введите новую цену (текущая: ${formatPrice(listing.price)})`, listing.price);
          if (newPrice !== null && !isNaN(newPrice) && newPrice > 0) {
            const allListings = getListings();
            const index = allListings.findIndex(l => String(l.id) === String(listingId));
            if (index !== -1) {
              allListings[index].price = parseInt(newPrice);
              localStorage.setItem('itmarket_listings', JSON.stringify(allListings));
              renderListingsTable();
              showToast('Цена изменена', 'success');
            }
          }
        });
      }
...
function router() {
  ...
  const routes = {
    ...
    'brands.html':   initBrandsPage,
    'listings.html': initListingsPage,
    'sell.html':     initSellPage,
  };
